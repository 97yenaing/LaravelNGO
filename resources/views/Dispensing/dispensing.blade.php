...
<div class="container containers page-color">
  <ul class="nav nav-tabs toggle dispencing_ul" id="hidden-title">
    ...
    <li class="nav-item">
      <a class="nav-link toggle-link " data-toggle="tab" href="#medical_entry" onclick="show_medic_entryList()">Stock
        Add List</a>
    </li>
  ...
  <div class="tab-content containers">
      ...
      <div class="consumption-Info">
        <div class="row dis-comsumMenu">
          ...
          <div class="col-md-3 dis-saveRefresh">
            ...
            <button class="btn btn-success dis-saveBtn save-batton" id="dis_save" onclick="dispencing_saveUpdate(this)"
              disabled="true">save</button>
            <button class="btn btn-success dis-saveBtn save-batton" id="dis_update"
              onclick="dispencing_saveUpdate(this)" style="display:none">Updated</button>
          </div>
        </div>
    ...
    <div id="Medical_add" class="tab-pane fade">
      ...
      <div class="row" id="medicineEdit_show">
        ...
        <div class="col-sm-2">
          <button class="btn btn-success dis-newItem save-batton" id="new_Medic" onclick="newItem_save(this)"
...
<script type="text/javascript">
  ...
  let patient_general;
  let updateSave;
  let updated_originData;
  let Updated_originStock; // to calculate stock and remaing stock in user interface
  let clinicCode,
  ...
  function disOtherChoice(fineID, otherID, orginalID, origianlClass) {
    ...
    if (consumType == "Clinic Out") {
      ...
      var $medic_other = $("<select>").attr({
        class: "form-select medicother_select",
        id: otherID
      });
      var other_optionMedic = ["HTY-C2(81)", "HTY-A(71)", "HTY-B(72)", "SPT(73)", "Winka(75)", "TBZY(76)", "PTO-DT(77)", "PTO-MCB(78)", "Hpakant(80)",
        "Taze(82)", "HTY-C1(83)", "Aye Nyein Myintar", "Lotus", "MMTN", "BAHAN", "Lab", "SDG", "TLL", "BKKe"
      ];
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        if (Object.keys(response).length == 1) {
          ...
          var patient_data = "Patient's Name =" + response[0]["Name"] + ",     " + "Father's Name =" + response[0]["Father"] + ",     " + "Fuchia ID =" +
            response[0]["FuchiaID"] +
  ...
  function check_medi() {
    ...
    if ($("#" + check_mediID).is(':checked')) {
      ...
      var quantity_div = $('<div>').attr({
        class: 'col-sm-6'
      }).append($('<label>').text("Quantity")).append(medi_quantity);
      var medistock_div = $('<div>').attr({
        class: "row st_qtRow" + check_mediID
      }).append(quantity_div);
      $("#" + check_mediID).parent().append(medistock_div.append(stock_div));
      var medic_nameList = $('<div>').attr({
        class: "medi-listName",
        id: "list_" + checkArea,
      }).append($('<label>').attr({
          class: "medic_nameNo"
        })
        .text(medic_count)).append($('<label>').attr({
          class: "medic_nameLabel"
        })
        .text($("#label" + check_mediID).text())).append($('<input>').attr({
          class: "medic-Lableqty",
          id: "medic_qtyLable"
        })
        .val(0)).append($('<button>')
        .text("Remove").attr({
          class: 'btn btn-remove',
          onclick: "remove_medicList()"
        }))
  ...
  function dispencing_saveUpdate(button) {
  ...
  function Quanttity() {
    ...
    for (var qChild = 0; qChild < qtyMedic_list.length; qChild++) {
      ...
      if (qtyamount == 0) {
        ...
        var medicQty_nameList = $('<div>').attr({
            id: "listQty_" + qtychild_number,
            class: "listQtyblock"
          })
          .append($('<label>').attr({
              class: "repaire_nameLabel",
              type: "number"
            })
            .text(qtyname)).append($('<input>').attr({
            class: "medic_repaireqty ",
            type: "number",
            placeholder: "Quantity",
            onChange: "medic_Quantity()",
            id: "reqty_" + qtychild_number,
          })).append($('<button>')
            .text("Remove").attr({
              class: 'btn btn-remove',
              onclick: "remove_medicList()"
    ...
    $("#saveQuantityList").append($('<button>').attr({
      onclick: "reparieOK()",
      class: "btn save-batton",
    }).text("OK").css({
      width: "20%"
    }));
  ...
  function reparieOK() {
    ...
    if (togo != "cancel") {
      $("#saveQuantity").hide();
      $("#medic_check").removeClass("freeze-body").css({
        opacity: "1"
      });
    }
  ...
  function checkStock_medi() {
    ...
    if ($("#" + stock_check).is(":checked")) {
      var stock_blog = $("<div>").attr({
          id: "stock_blog" + idnumber,
          class: "row"
        }).append($("<div>").attr({
            class: "col-sm-2"
          })
        ...
        .append($("<div>").attr({
          ...
          .append($("<button>").attr({
            class: "btn btn-warining save-batton dis-addStock",
            onclick: "stockAdd(this)",
            id: "sto_add" + idnumber
          }).text("ADD")));
  ...
  function stockAdd(button) {
    ...
    clearTimeout(timeoutHandle);
        if (response[0]==1) {
          ...
          console.log(response);
          $("#stock_blog" + idnumber).remove();
          $("#checkStock" + idnumber).prop({
          "checked": false
          })
        } else {
          alert("Fail Stock Add");
        }
      ...
      success: function(response) {
        ...
        if (response[0].length > 1) {
          ...
          $.each(response[0], function(index, item) {
            var medic_update_show_list = $("<div>").attr({
                class: "row show-update-data"
              })
              .append($("<div>").attr({
                class: "col-sm-1"
              }).text(index + 1))
              ...
              .append(
                ...
                .append($("<button>").attr({
                  class: "btn btn-info btn-warining",
                  id: "medic_update_" + index,
                  onclick: "show_update_data()"
                }).text("To-Update"))
              )
  ...
  function show_update_detail(update_show_detail) {
      ...
      if (index != 0 && index % 2 == 0) {
        ...
        var medic_nameList = $('<div>').attr({
            ...
            .attr({
              class: "medic-Lableqty",
              id: "medic_qtyLable",
              onChange: "updateQtyCheck()"
            }).val(item))
          .append($('<button>').text("Remove").attr({
            class: 'btn btn-remove',
            onclick: "remove_medicList()"
          }))
    ...
    $(".medic-searchUpdate").css({
      opacity: "",
      top: ""
    }).addClass("freeze-body");
    $(".consumption-Info").css({
      position: "",
      opacity: ""
    }).removeClass("freeze-body")
  ...
  function addEdit() {
    ...
    if (editAdd == "Add") {
      ...
      $("#medicineEdit_show div:nth-child(2) input").attr({
        id: "medicine_newname"
      });

      $("#Edit_medic,.checkEdit_mediSection").hide();
      $("#new_Medic").show();
      var addEdit_medic = {
        ...
        if (response[0].length > 0) {
          ...
          for (let index = 0; index < response[0].length; index++) {
            ...
            var tableBody =
              ...
              "</ul>" +
              "</td>" +
              "<td><button class='btn btn-danger' onclick='remove_medical_list(this)' id=medical_remove_" + response[0][index]["id"] +
              ">Delete</button></td>" +
              "</tr>";
      ...
      success: function(response) {
        ...
        if (response[0].length > 0) {
          ...
          $.each(response[0], function(index, medicine) {
            var entry_data = $("<div>").attr({
                class: "row medical-entry-list"
              })
              .append($("<div>").attr({
                class: "col-sm-1"
              }).text(index + 1))
              .append($("<div>").attr({
                class: "col-sm-7"
              }).text(itemsArray[medicine["Serial Number"]]["Medic_item"]))
              .append($("<div>").attr({
                class: "col-sm-2"
              }).text(medicine["Arrival_Date"]))
              .append($("<div>").attr({
                class: "col-sm-2"
