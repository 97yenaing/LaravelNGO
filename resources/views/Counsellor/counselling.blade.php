...
<script type="text/javascript" language="javascript">
  ...
  let ddDate=0;
  let service ='';
  let patient_generalInfo=[]; let Age=0; let hts_row_address=0;
  let resp=0;
  let address=0;
    ...
    $.ajaxSetup({
      ...
      $.ajax({
                ...
                success:function(response){
                  ...
                  var hts_remain_list=$("<div>").attr({class:'row remain_data'})
                      .append($("<div>").attr({class:'col-sm-1'}).text(hts_count+1))
                      .append($("<div>").attr({class:'col-sm-2'}).append($("<button class='btn btn-info'  onclick='To_entry_section()'>").text(response[0][hts_count]["CID"])))
                      .append($("<div>").attr({class:'col-sm-2'}).text(response[0][hts_count]["agey"]))
                      .append($("<div>").attr({class:'col-sm-2'}).text(response[0][hts_count]["Gender"]))
                      .append($("<div>").attr({class:'col-sm-2'}).text(response[0][hts_count]["Patient_Type"]))
                      .append($("<div>").attr({class:'col-sm-2'}).text(response[0][hts_count]["vdate"]));
              ...
              for (var i = 0; i < response[0].length; i++) {
                ...
                if(response[1]==0){
                  ...
                  "<tr style='background-color:#A7DBD8; color:#000000;'"+ "id='"+rowName+"'>"
                    ...
                    +"<td class= tablet-pc id='"+btnName+"'>"+"<button class='btn btn-info btn-warning' id='counselling_update_"+response[0][i]['id']+"'  onclick='updateFiller()'  >"+ "Update"+"</button>"+
                      "<button class='btn btn-info btn-danger counsel-delete' id='counselling_remove_"+response[0][i]['id']+"' onclick='remove_row()'  >"+ "Delete"+"</button>"+"</td>"
                  +"</tr>";
                }else{
                  var result_body1 =
                  "<tr style='background-color:#A7DBD8; color:#000000;'"+ "id='"+rowName+"'>"
  ...
  function remove_row(){
    ...
    if(confirm("Are you sure to delete this data")){
      ...
      $.ajax({
            ...
            success:function(response){
              alert(response);
              if(response!="Your Wrong Credential contant to Admin"&&response!="You don't have permission to delete this record"){
                HTS_list();
              }
            }
          })
  ...
  function Save_and_Update(){
        ...
        if($("#c2_done").prop("checked")){
          c2_done=1;
        }else{
          c2_done=0;
        }
        if($("#stable").prop("checked")){
          stable=1;
        }else{
          stable=0;
        }
        if($("#phq4").prop("checked")){
          phq4=1;
        }else{
          phq4=0;
        }
        if($("#gad7").prop("checked")){
          gad7=1;
        }else{
          gad7=0;
        }
        if($("#brest_cancer").prop("checked")){
          brest_cancer=1;
        }else{
          brest_cancer=0;
        }
        if($("#hepC").prop("checked")){
          hepC=1;
        }else{
          hepC=0;
        }
        if($("#art_ost").prop("checked")){
          art_ost=1;
        }else{
          art_ost=0;
        }
        ...
        if(hts_counselling==1||counsellingOnly==1||pt_data_update=="Only Patient Info_Update"||pt_data_update=="Update"){
          ...
          if(pt_data_update=="Only Patient Info_Update"||pt_data_update=="Update"){
            hts_counselling=0;counsellingOnly=0;
          }
          ...
          if(hts_OK||counsellingOnly==1||pt_data_update=="Only Patient Info_Update"||pt_data_update=="Update"){
            col_data={
            ...
            if($("#riskChangeLab").val()=="No"){
              col_data['labTestDate']=Counselling_Date;
                ...
                col_data['labTestDate'] =col_data["risk_change_date"]= formatDate(document.getElementById("labTestDate").value);
                  ...
                  //  processData:false,
                    ...
                    success:function(response){
                      ...
                      console.log(response[0].length+"hello length");
                        ...
                        case 3:alert("This Patient can not update,contant to Admin");break;
                        case 3.2:alert("Your Update is not complete,'Fail'");break;
                        }
  ...
  function risk_update(){ // to update Data by Counsellor
    ...
    var  data={
        ...
        data["register_age"]=$("#agey_register").val();
        data["change_risk_rason"]=$("#risk_change_resason").val();
