...
<body>
  <form action={{ route('hts_calreport') }} method="post">
    @csrf
      ...
      <div class="row" style="justify-content: center">
        <div class="col-sm-2">
          ...
          <div class="date-holder">
            <input type="text" id="from_date" name="from_date" class="form-control Gdate" {{ old('from_date') !=null
              ? 'value=' . old('from_date') : '' }} placeholder="dd-mm-yyyy">
            <img src="../img/calendar3.svg" class="dateimg" alt="date">
          </div>
          @error('from_date')
          <div class="alert alert-danger">မှန်ကန်သော Date ထည့်သွင်းပေးပါ</div>
          @enderror
        </div>
        <div class="col-sm-2">
          <label for="" class="form-label">To Date</label>
          <div class="date-holder">
            <input type="text" id="to_date" name="to_date" class="form-control Gdate" {{ old('to_date') !=null
              ? 'value=' . old('to_date') : '' }} placeholder="dd-mm-yyyy">
            <img src="../img/calendar3.svg" class="dateimg" alt="date">
          </div>
          @error('to_date')
          <div class="alert alert-danger">မှန်ကန်သော Date ထည့်သွင်းပေးပါ</div>
        ...
        <div class="col-sm-2">
          <select name="do_type" id="hts_report_do" class="form-select" style="margin-top: 33px">
            <option value="export">Export</option>
            <option value="view">Only View</option>
          </select>
        </div>
        <div class="col-sm-2 hts-reportBtn">
          <button class="btn update-batton counsel-report-btn">Report</button>
        </div>
      </div>
      @if($hts_result)
...
@endsection
<script>
  let input_data = @json($input_data);
  document.addEventListener("DOMContentLoaded", function() {
   
    if (input_data) {
      console.log("ok if")
      document.getElementById('from_date').value =input_data["from_date"];
      document.getElementById('to_date').value =input_data["to_date"];
    }
  });
