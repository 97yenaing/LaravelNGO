...
<form action="{{ route('risk_log_data') }}" method="post" class="container containers">
  ...
  <div  class="page-color riskLog-section" >
    <div class="row">
      ...
      <div class="col-sm-2 risklog-date" style="{{ old('searchType') == 'Date'||old('searchType') == null ? 'display:block' : 'display:none' }}">
        <label for="riskLog_From" class="form-label">From Date</label>
        <div class="date-holder">
            <input type="text" class="form-control Gdate" value="{{ old('riskLog_From') }}" name="riskLog_From" id="riskLog_From" placeholder="dd-mm-yyyy">
      ...
      <div class="col-sm-2 risklog-date" style="{{ old('searchType') == 'Date' ||old('searchType') == null ? 'display:block' : 'display:none' }}">
          ...
          <div class="date-holder">
              <input type="text" class="form-control Gdate" value="{{ old('riskLog_To') }}" name="riskLog_To" id="riskLog_To" placeholder="dd-mm-yyyy">
      ...
      <div class="col-sm-2 riskLog-id"  style="{{old('searchType') == 'ID' ? 'display:block' : 'display:none' }}">
        ...
        <input type="number" class="form-control" name="riskLog_searchID" value="{{ old('riskLog_searchID') }}" id="riskLog_id">
      ...
      <div class="col-sm-2">
        <select class="form-select" name="export_view" id="risklog_wantCategory" style="margin-top: 35px;background-color: #c4b20f">
            <option value="Export" {{ old('export_view') == 'Export' ? 'selected' : '' }}>Export Risk Log</option>
            <option value="riskLog_view" {{ old('export_view') == 'riskLog_view' ? 'selected' : '' }}>Only Risk log View</option>
        </select>
    </div>
    ...
    @if ($final_log!=null)
      ...
      <div class="row" style="text-align: center">
          ...
          <div class="col-sm-2"><h3>Change Date</h3></div>
          <div class="col-sm-2"><h3>Old Risk</h3></div>
          <div class="col-sm-2"><h3>Current Risk</h3></div>
          <div class="col-sm-2"><h3>Due to Patient</h3></div>
          <div class="col-sm-2"><h3>Change User</h3></div>
      </div>
...
<script type="text/javascript">
  ...
  function risk_change_date(){
    ...
    if(changeDate.length>5){
      $("#old_risk").text(final_log[gid][changeDate]["Old Risk"]);
      $("#current_risk").text(final_log[gid][changeDate]["Current Risk"]);
      $("#due_patient").text(final_log[gid][changeDate]["Due_to_patient"]);
      $("#change_user").text(final_log[gid][changeDate]["change_user"]);
    }else{
      $("#old_risk,#current_risk,#due_patient,#change_user").text("");
    }
