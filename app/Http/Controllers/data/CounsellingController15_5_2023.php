  ...
  public function save_data(Request $request){
      ...
      if($hts_counselling==1){ // to save data in  table (config and counselling table and HTS)
        ...
        if($follow_lastDate == $cdate){
          ...
          ->latest('created_at')
          ->value('created_at');
          if($hiv == $request -> hiv_test_date || $hebc == $request -> hep_date  || $rpr ==$request -> syp_date) {
            ...
            Followup_general::where('Pid',$gid)->latest('Visit Date')
              ...
              Coulselling ::// HTS Create row input
                ...
                CounsellorRecords :://where('Pid','=',$gid)->where('Counselling Date','=',$hiv_test_date)
                create([
                "Clinic Code" => $request -> clinic_code ,
                "Pid" => $request -> Pid,
                "FuchiaID" => $request -> FuchiaID ,
                ...
                "Counselling Date" => $request -> Counselling_Date,
                "Counsellor" => $request -> Counsellor,
                "Main Risk" => $request -> Main_Risk,
                "Sub Risk" => $request -> Sub_Risk,
                ...
                "Pre" => $request -> Pre,
                "Post" => $request -> Post,
                "HTSdone" => $request -> HTSdone,
                "Reason" => $request -> Reason,
                "Status" => $request -> Status,
                "PrEP" => $request -> PrEP ,
                "PrEP Status" => $request -> PrEP_Status,
                "C1" => $request -> c1,
                "C2" => $request -> c2,
                "C3" => $request -> c3,
                "ADH" => $request -> adh,
      ...
      if($listShow == 1){
        ...
        $counselling_data = CounsellorRecords::whereBetween('Counselling Date', [$date_from, $date_to])->get();
        //   $hts_data = Coulselling::where(function($query) use ($hiv_test_date, $other_date) {
        //     $query->where('Visit_date', '=', $hiv_test_date)
        //           ->orWhere('Visit_date', '=', $other_date);
        // });
      ...
      if($decryptFetch == 1){// return data from view to encrypt the row
        ...
        "EAC","HMT","C P case","PMTCT",)->where('Pid', '=', $id)->where('Counselling Date',$res_date)->orderByDesc('created_at')
        ->take(4)
        ->get()
        ->reverse();
        $table="General";
        $hTSdone=$prep_status=$reason=$status="";
      ...
      if($htsUpdate == 1){ // to update HTS data rows
        ...
        // ->exists();
          ...
          if($hiv_exist||$hebc_exist||$rpr_exist ){
              ...
              ->exists();
              if($hts_exist){
                $coun_record_exist=CounsellorRecords::where('Pid',$gid)->where('Counselling Date',$cdate)->exists();
                if($coun_record_exist){
                  ...
                  CounsellorRecords::where('Pid',$gid)->where('Counselling Date',$cdate)
                  ->update([
                    "Counselling Date" => $request->Counselling_Date,
            ...
            if($found){
              ...
              $lab_updated_fine="no";
                ...
                $labs_updated = [ $hiv_res, $rpr_value, $hbc_res ,$urine_res, $oi_Lab, $sti_lab_res , $afb_res, $general_res, $stool_res, $covid_res, $Vir_res ];
                  ...
                  if($lab_updated_fine=="yes"||$labTestDate=="preTest"||$labTestDate=="postTest"){
