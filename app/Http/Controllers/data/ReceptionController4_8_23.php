...
class ReceptionController extends Controller
{
  ...
  public function general_patients(){
    ...
    if($ckID ==1){ //to check the patient is in general patients list
        ...
        if($patientData != null){
          ...
          return response()->json([
            $patientData,$ptNameDecrypt,$ptFather,$dob,$region,$town,$quarter,$followupData,$gender,$height
   ...
   { $table = "General";
     ...
     'fmaplancheck','gneralcheck','ncdcheck','hivTBcheck','fcentercheck','labInvestcheck','pha_new_old','pha_cohort','prep_new_old',
     ...
     $finalString=$diagnosis_encrypt['phacheck'].'-'.$diagnosis_encrypt['pha_new_old'].'-'.$diagnosis_encrypt['pha_cohort'].'/'
      .$diagnosis_encrypt['artcheck'].'-'.$diagnosis_encrypt['art_new_old'].'-'.$diagnosis_encrypt['art_cohort'].'/'
      .$diagnosis_encrypt['prepcheck'].'-'.$diagnosis_encrypt['prep_new_old'].'/'
      .$diagnosis_encrypt['pmtctcheck'].'-'.$diagnosis_encrypt['pmtct_new_old'].'/'
      .$diagnosis_encrypt['anccheck'].'-'.$diagnosis_encrypt['anc_new_old'].'/'
      .$diagnosis_encrypt['fmaplancheck'].'-'.$diagnosis_encrypt['famaplan_new_old'].'/'
      .$diagnosis_encrypt['gneralcheck'].'-'.$diagnosis_encrypt['general_new_old'].'-'.$diagnosis_encrypt['general_diagnosis'].'/'
      .$diagnosis_encrypt['ncdcheck'].'-'.$diagnosis_encrypt['ncd_new_old'].'-'.$diagnosis_encrypt['ncd_diagnosis'].'-'.$diagnosis_encrypt['ncd_drugSupply'].'/'
      .$diagnosis_encrypt['hivTBcheck'].'-'.$diagnosis_encrypt['hivTB_new_old'].'/'
      .$diagnosis_encrypt['fcentercheck'].'-'.$diagnosis_encrypt['feedcentre_new_old'].'/'
      .$diagnosis_encrypt['labInvestcheck'].'-'.$diagnosis_encrypt['labInvest_new_old'].'/';
   ...
   {  
     ...
     $show_resultNext2=[];
     $count=1;
     $next_data=Followup_general::select('id','Pid','Pateint_Diagnosis','Patient Type')
                      -> where('Next Appointment Date','=',$nDate)
                      ->get();
    
      for($p=0;$p<count($next_data);$p++){
        if($next_data[$p]['Patient Type']==null){
          ...
          $test_name=['PHA','ART','PrEP','PMTCT','ANC','Family planing','General','NCD','hivtb','FC','lab_iv_only'];
              if($next_data[$p]['Pateint_Diagnosis']!=null){
                ...
                if($final_testType[$visit_type]!="false"){
      ...
      return response()->json([
        $show_resultNext,$show_resultNext2,$next_data,
   ...
   public function followup_update_filler($request)//12
   {
      ...
      if($toUpdateFollowup==1){
       ...
       $table = "General";
       $other_decrypt['gender']=Crypt::decrypt_light($followupData[0]["Gender"],$table);
      ...
      return response()->json([
       $followupData,$other_decrypt,$diagnosis_final,
       ]);
     }
   }
   ...
   public function export(Request $data)
   {
     ...
     $to = $date_to->format('Y-m-d');
        ...
        $users1= Followup_general::
              ...
              //return Excel::download(new ReceptionExport($users,$users1,$users2), 'followup_data.xlsx');
        }
