  ...
  public function export(Request $data)
  {
    ...
    $users = $model->whereBetween('vdate', [$from, $to])->with([
      'ptconfig' => function ($query) {
        $query->select("Pid", 'Name', 'Township', 'Region', 'Quarter', 'Date of Birth', 'Agey', 'Agem', "Main Risk", "Sub Risk", "Gender", 'Risk Log', 'Risk Change_Date', 'Former Risk'); // Select the specific columns from ptconfig
      }
    ])->get();
    if ($testName == "hep_bc") {
