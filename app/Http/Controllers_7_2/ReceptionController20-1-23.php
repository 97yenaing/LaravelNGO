...
class ReceptionController extends Controller
{
 ...
 // &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
  ...
  public function hty_A_Recession_data(Request $request){
      ...
      if($gtReg == 1){
        $lastVisitData = PtConfig::orderBy('id', 'desc')->limit(1)->get();
        if(count($lastVisitData)>0){
          $lastVisitID = $lastVisitData["Pid"];
          $lastVisitDate = $lastVisitData["Visit Date"];
        }else{
        	$lastVisitID =0; $lastVisitDate = 0;
        }
        if($lastVisitID == $gid && $lastVisitDate == $vDate ){ // This condition is to protect Duplicate entry in a day;
          $gtReg = 0;
          $DuplicateInput = true;
           return response()->json([
             $DuplicateInput
      ...
      if($ptFup==1){ // follow up register
        $lastVisitData = Followup_general::orderBy('id', 'desc')->limit(1)->get();
        if(count($lastVisitData)>0){
          $lastVisitID = $lastVisitData[0]["Pid"];
          $lastVisitDate = $lastVisitData[0]["Visit Date"];
        }else{
          $lastVisitID=0;$lastVisitDate=0;
        }
        if($lastVisitID == $gid && $lastVisitDate == $vDate){ // This condition is to protect Duplicate entry in a day;
