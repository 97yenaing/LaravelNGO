...
class LabExport implements FromView, WithColumnFormatting
{
  ...
  public function view(): View
  {
        ...
        if (!array_key_exists($user['ptconfig']['Pid'], $final_log) && $user['ptconfig']['Risk Log'] != null) {
          ...
          if ($user['ptconfig']['Risk Change_Date'] != null && $user['ptconfig']['Former Risk'] != null && $user['ptconfig']['Former Risk'] != "731") {
            ...
            if ($vdate < $riskChangeDate) {
              $user['Patient_Type'] = $user['ptconfig']['Former Risk'];
              $user['Patient Type Sub'] = '';
            }
          }
        }
        if (array_key_exists($user['ptconfig']['Pid'], $final_log)) {
          foreach (array_reverse($final_log[$user['ptconfig']['Pid']][$user['ptconfig']['Pid']]) as $date => $data) {
            if (strlen($date) == 10) {
              ...
              if ($vdate < $riskChangeDate) {
                $user['Patient_Type'] = Crypt::encrypt_light($data['Old Risk'], 'General');
                $user['Patient Type Sub'] = Crypt::encrypt_light($data['Old Sub Risk'], 'General');
              }
            }
          }
        }
    ...
    switch ($this->testName) {
      case 'hiv':
        $encrypted_columns = ['Gender', 'Patient_Type', 'Patient Type Sub', 'Detmine_Result', 'Unigold_Result', 'STAT_PAK_Result', 'Final_Result', 'Req_Doctor', 'Counsellor', 'LabTech'];

        $date_type = ['vdate', 'bcollectdate'];
        break;
      case 'rpr':
        $encrypted_columns = ['Gender', 'RDT(Yes/No)', 'RDT Result', 'Quantitative(Yes/No)', 'RPR Qualitative', 'Patient_Type', 'Patient Type Sub', 'Titre(current)', 'Titre(Last)', 'Req_Doctor', 'Counsellor', 'Lab Tech'];
        $date_type = ['vdate', 'TitreLastDate', 'Issue Date'];
        break;
      case 'sti':
        $encrypted_columns = ['Gender', 'Req_Doctor', 'Requested Doctor New', 'Patient_Type', 'Patient Type Sub', 'Wet Mount clue cell', 'Wet Mount Trichomonas', 'Wet Mount candida', 'wetoth', 'urethra WBC', 'Urethra diplococci intra-cell', 'Urethra diplococci extra-cell', 'Urethra Candida', 'uoth', 'Fornix Clue Cells', 'Fornix WBC', 'Fornix diplococci intra-cell', 'Fornix diplococci extra-cell', 'Fornix Candida', 'pfother', 'Endo cervix WBC', 'Endo cervix diplococci intra-cell', 'Endo cervix diplococci extra-cell', 'Endo cervix Candida', 'eother', 'Rectum WBC', 'Rectum diplococci intra-cell', 'Rectum diplococci extra-cell', 'rother', 'First Per Urine', 'Epithelial cells', 'PMNL cells', 'First Per Urine Diplococci Intra-Cell', 'First Per Urine Diplococci Extra-Cell', 'fpu_oth', 'Lab Techanician', 'Other Bacteria'];
        $date_type = ['vdate', 'idate'];
        break;
      case 'hep_bc':
        $encrypted_columns = ['Gender', 'Requested Doctor old', 'Req_Doctor', 'Patient_Type', 'Patient Type Sub', 'Hiv status', 'HepB Test', 'HepB TOT', 'HepB Result', 'HepC Test', 'HepC TOT', 'HepC Result', 'Lab Tech'];
      ...
      case 'urine':
        ...
        break;
      case 'oi':
        $encrypted_columns = ['Gender', 'Req_Doctor', 'Patient_Type', 'Patient Type Sub', 'TB_LAM_Report', 'Serum Result', 'serum_pos', 'CSF for Cryptococcal Antigen', 'csf_crypto_pos', 'csf_fungal', 'CSF Smear Giemsa Stain', 'CSF Smear India Ink', 'skin_fungal', 'Skin Smear Giemsa Stain', 'Skin Smear India Ink', 'lymph_test', 'Lymph Giemsa Stain', 'Lymph India Ink', 'Lab Techanician', 'Toxo plasma', 'Toxo igG', 'Toxo igM'];
        $date_type = ['vdate', 'issued'];
        break;
      case 'general':
        $encrypted_columns = ['Gender', 'Requested Doctor old', 'Req_Doctor', 'Patient_Type', 'Patient Type Sub', 'Dangue RDT', 'NS1 Antigen', 'IgG Result', 'IgM Result', 'Malaria RDT', 'Malaria RDT Result', 'Malaria_spec', 'Malaria_grade', 'Malaria_stage', 'malaria_microscopy', 'Malaria Microscopy Result', 'RBS test', 'RBS', 'FBS test', 'FBS', 'haemo_done', 'haemoglobin', 'hba1c', 'Lab Tech'];
        $date_type = ['vdate', 'Issue Date'];
        break;
      case 'stool':
        $encrypted_columns = ['Gender', 'Req_Doctor', 'Patient_Type', 'Patient Type Sub', 'Clinic', 'st_stool', 'st_colour', 'wbc_pus_cell', 'st_consistency', 'st_rbcs', 'st_other', 'st_comment', 'st_lab_tech'];
      ...
      case 'afb':
        $encrypted_columns = [
          ...
          'Region',
          'Quarter',
          'Name', //confidential must be last
        ];
        $date_type = ['vdate', 'speci_receive_dt1', 'speci_receive_dt2', 'afb_issue_date'];
        break;
      case 'covid19':
        $encrypted_columns = ['Gender', 'Req_Doctor', 'Patient_Type', 'Patient Type Sub', 'co_Age', 'type_of_patient_covid', 'specimen_type', 'co_test_type', 'covid_result', 'covid_lab_tech'];
        $date_type = ['vdate', 'covid_issue_date'];
        $this->testName = 'covid';
        break;
      case 'viral':
        $encrypted_columns = ['Gender', 'Patient_Type', 'Patient Type Sub', 'Req_Doctor', 'Sample Sent to', 'Viral Load Result', 'Other org code', 'Detect'];
